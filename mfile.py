...
if not 'SDE_BUILD_KIT' in os.environ:
    ...
    sde_bk_dir = os.path.join(dir, 'sde')
    print(os.path.dirname(dir))
    print(dir, sde_bk_dir)
...
# Tools sources
tool_sources = {}
tool_sources['MAPProfiler'] =  [ os.path.join('src', p) for p in os.listdir('src') ] 
tool_sources['MAPProfiler'].append('MAPProfiler.cpp')

# Programs sources
programs_sources = {}
# if env.on_linux():
#     programs_sources['dcfg-reader'] =  ['dcfg-reader.cpp']
#     programs_sources['proxy-dcfg'] =  ['proxy-dcfg.cpp']
